...
def api_get_messages(chat_id: int, user: User = Depends(current_user_required)):  # >>> changed
    ...
    return [
        {"id": m.id, "role": "user" if m.role else "ai", "content": m.content, "timestamp": m.timestamp.isoformat()}
        for m in msgs
    ]

@app.post("/api/chats/{chat_id}/messages")
def api_post_message(chat_id: int, payload: PostMessageIn, user: User = Depends(current_user_required)):  # >>> changed
    with Session(engine) as session:
        chat = session.get(ChatFolder, chat_id)
        if not chat or chat.user_id != user.id:
            raise HTTPException(status_code=404, detail="Not found")
        ...
        ai_text = "✅ Noted: " + payload.content  # placeholder AI
        ai_msg = Message(chat_folder_id=chat.id, role=False, content=ai_text)
        session.add(ai_msg)
        session.commit()
