...
<body>
  ...
  <div class="register-container">
    <div class="icon-container">
      <lord-icon src="https://cdn.lordicon.com/kdduutaw.json" trigger="loop"
        stroke="bold" state="hover-looking-around"
    ...
    <form id="registerForm">
      <div class="form-group">
        ...
        <input id="username" type="text" class="form-input" placeholder="Enter your username" required />
      </div>
      <div class="form-group">
  ...
  <script>
    ...
    document.getElementById('registerForm').addEventListener('submit', function(e) {
      ...
      let valid = true;
      [username,email,password].forEach(i => i.classList.remove('error','success'));
      if (username.value.length < 3) { username.classList.add('error'); valid = false; } else username.classList.add('success');
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) { email.classList.add('error'); valid = false; } else email.classList.add('success');
      if (password.value.length < 6) { password.classList.add('error'); valid = false; } else password.classList.add('success');
      if (valid) {
        btn.textContent = 'Registering...'; btn.disabled = true;
        setTimeout(()=>{ btn.textContent='Registration Successful!'; btn.style.background='#27ae60';
          setTimeout(()=>{ btn.textContent='Register'; btn.style.background='#3F72AF'; btn.disabled=false; this.reset(); [username,email,password].forEach(i=>i.classList.remove('error','success')); },2000);
        },2000);
      }
    });
  </script>
